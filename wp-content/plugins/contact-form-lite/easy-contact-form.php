...
function ecf_run() {
	...
	if ( ecf_is_gutenberg() ) include_once plugin_dir_path( __FILE__ ) . 'inc/ecf-block/init.php';
...
function easycontactform_columns_edit_columns_list( $ecf_columns, $post_id ){
	...
	switch ( $ecf_columns ) {
		...
		echo '<a class="ecf_tooltips" alt="Edit Form" href="'.get_edit_post_link( $post_id ).'"><span class="dashicons dashicons-edit ecf_form_actions"></span></a>'.( current_user_can('edit_posts') ? '<a class="ecf_tooltips" alt="Duplicate Form" href="'.admin_url( 'admin.php?action=ecf_duplicate_form&amp;post=' . $post_id . '&amp;nonce='.wp_create_nonce( 'ecf-duplicate-nonce' ).'"><span class="dashicons dashicons-admin-page ecf_form_actions"></span></a>' ) : '').'<a style="cursor:pointer;" class="ecf_tooltips" alt="Preview" onClick="alert(\'This feature only available in Pro Version.\')"><span class="dashicons dashicons-desktop ecf_form_actions"></span></a><a class="ecf_tooltips delforms" alt="Delete Form" href="'.( isset( $_GET['post_status'] ) && $_GET['post_status'] == 'trash' ? get_delete_post_link( $post_id, '', true ) : get_delete_post_link( $post_id ) ).'"><span class="dashicons dashicons-trash ecf_form_actions"></span></a>';
