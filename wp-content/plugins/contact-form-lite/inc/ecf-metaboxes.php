				...
				if ( ecf_check_browser_version_admin( get_the_ID() ) != 'ie8' ) {
					...
					foreach ( $field['options'] as $key => $option ){
						...
						if ( $meta ){
							...
							echo '<div class="ecf_redirect_opt_cont istop"><span>Text</span><input class="redirectinpt" type="text" name="ecf_meta['. $field['id'] .'][]" id="'. $field['id'] .'_text" value="'. ($meta ? $meta[1] : $field['txt']) .'" size="45" /></div><div class="ecf_redirect_opt_cont"><span>URL</span><input type="text" class="redirectinpt" name="ecf_meta['. $field['id'] .'][]" id="'. $field['id'] .'_url" value="'. ($meta ? $meta[2] : $field['url']) .'" size="45" /></div></td>';
	...
	$dataTemp = $_POST['ecf_meta'];
		...
		foreach( $dataTemp as $key => $val ) {
			...
			if ( is_array( $dataTemp[$key] ) ) {
				...
				if ( $key === 'ecf_formbuilder_format' ) $dataTemp[$key] = $dataTemp[$key];
				else
