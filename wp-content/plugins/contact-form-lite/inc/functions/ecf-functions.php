...
function ecf_duplicate_form(){
	...
	if ( ( isset( $_GET['nonce'] ) && ! wp_verify_nonce( $_GET['nonce'], 'ecf-duplicate-nonce' ) ) || ! is_admin() || ! current_user_can( 'manage_options' ) ) {
...
function ecf_get_forms() {
	...
	if ( $gt ) {
		foreach ( $gt as $ce ) {
			$forms[] = array( 'id' => ''.$ce->ID.'', 'name' => $ce->post_title );
		}
	}
...
function ecf_posts_notify( $hook ) {
	...
	if ( get_option( 'ecf_block_notify' ) == '' ) {
		...
		wp_localize_script( 'ecf-post', 'ecf_popup', array( 'content' => $cnt ) );
